...
// Replace with your actual local makeup banner image asset path later
import MakeupHeroImg from "../../../assets/images/makeup/makeup-banner.png";

export default function MakeupHero() {
  const handleScrollToProducts = () => {
    const productSection = document.getElementById("makeup-grid");
    if (productSection) {
  ...
  return (
    <section className="w-full bg-[#FFFBF7] py-12 md:py-20 px-6 lg:px-12 border-b border-[#F1E7DE]">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-12 items-center">
        ...
        <div className="flex flex-col space-y-5 sm:space-y-6 text-left order-2 md:order-1">
          <div className="space-y-2">
            ...
            <h1 className="font-serif text-[#4A3B32] font-bold text-4xl sm:text-5xl lg:text-6xl tracking-tight leading-[1.1]">
              Glow & Glam <br />
              <span className="text-[#E07A5F]">Collection</span>
            </h1>
          </div>

          <p className="text-[#705A4F] font-sans font-medium text-sm sm:text-base leading-relaxed max-w-md">
            Discover your perfect makeup look with weightless, ultra-pigmented formulations designed to enhance your natural beauty all day long.
          </p>
        ...
        {/* Right Column: High-Fashion Portrait Image Frame */}
        <div className="order-1 md:order-2 w-full flex justify-center">
          <div className="relative w-full max-w-md md:max-w-none aspect-[4/5] rounded-[2.5rem] overflow-hidden border border-[#F2E3D5] bg-white p-3 shadow-xs group">
            <div className="w-full h-full rounded-[2rem] overflow-hidden bg-[#FFFBF7] relative">
              <img
                src={MakeupHeroImg}
                alt="Elegant Professional Makeup Look Portfolio"
                className="w-full h-full object-cover group-hover:scale-102 transition-transform duration-700"
                draggable="false"
                onError={(e) => {
                  // Graceful structural fallback placeholder if image path isn't mapped yet
                  e.target.src = "https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?q=80&w=1000";
                }}
              />
              {/* Subtle ambient lighting inner shade overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-[#4A3B32]/10 via-transparent to-transparent pointer-events-none"></div>
            </div>

            {/* Decorative organic background shape ring */}
