...
export default function CategoryCollectionCard({ title, description, image, onClick }) {
  return (
    <div className="bg-[#FFF6F0] rounded-2xl p-6 border border-[#F0E2D5] flex flex-col items-center text-center shadow-xs hover:shadow-md transition-all duration-300">
      ...
      <button 
        onClick={onClick}
        className="bg-[#E07A5F] text-[#4A3B32] font-serif font-bold text-xs px-6 py-2 rounded-xl shadow-xs hover:bg-[#d0694e] transition duration-200 cursor-pointer"
      >
        Explore
      </button>
